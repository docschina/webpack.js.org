...
webpack 4 默认使用 `stats.toJson()` 输出大量数据。但是除非在增量步骤中做必要的统计，否则请避免获取 `stats` 对象的部分内容。`webpack-dev-server` 在 v3.1.3 以后的版本，包含一个重要的性能修复，即最小化每个增量构建步骤中从 `stats` 对象获取的数据量。
...
尽量在生成入口 chunk 减小其体积以提高性能。下面的配置为运行时代码创建了一个额外的 chunk，所以它的生成代价较低：
...
- `node-sass` 中存在 bug，会减少 Node.js 线程池中的线程。当使用 `thread-loader` 时，需要设置 `workerParallelJobs: 2`。
